...
describe('rules/file-ts-naming', () => {
  it('accepts camelCase .ts file', () => {
    const ctx = runRule(fileTsNaming as any, '/home/cgs/project/src/myFile.ts')
    expect(ctx.report).not.toHaveBeenCalled()
  })

  it('accepts camelCase .tsx in nested folder', () => {
    const ctx = runRule(fileTsNaming as any, '/home/cgs/project/src/components/myComponent.tsx')
    expect(ctx.report).not.toHaveBeenCalled()
  })
  ...
  it('ignores files specified in options', () => {
    ...
    const ctx = runRule(fileTsNaming as any, '/home/cgs/project/src/shims-vue.d.ts', [{ ignores: ['shims-vue.d.ts'] }])
    expect(ctx.report).not.toHaveBeenCalled()
  })

  it('reports dashed filenames', () => {
    const ctx = runRule(fileTsNaming as any, '/home/cgs/project/src/my-file.ts')
  ...
  it('reports underscored filenames', () => {
    const ctx = runRule(fileTsNaming as any, '/home/cgs/project/src/my_file.ts')
    expect(ctx.report).toHaveBeenCalled()
    const call = ctx.report.mock.calls[0][0]
    expect(call.messageId).toBe('filenameNotCamel')
